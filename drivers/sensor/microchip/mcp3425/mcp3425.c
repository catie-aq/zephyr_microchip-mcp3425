			       ...
			       struct sensor_value *val)
{
	...
	val->val2 = (data->voltage_uv % MCP3425_VOLTAGE_DECIMAL) /
		    10; // fractional part, must be 10^(-6) and not 10^(-7) !
...
static int mcp3425_init(const struct device *dev)
{
	...
	LOG_DBG("PGA gain=%d, max input voltage is %dmV, lsb is %d.%duV.", cfg->adc_pga_gain,
